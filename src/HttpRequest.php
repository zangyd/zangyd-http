...
use Zangyd\Util\ZangydHttp\Http\Request;
use Zangyd\Util\ZangydHttp\Http\Psr7\UploadedFile;
...
class HttpRequest
{
     ...
     * 处理器
     *
     * @var \Zangyd\Util\ZangydHttp\Handler\IHandler
     */
    private $handler;
    ...
    /**
     * 发送内容，可以是字符串、数组（支持键值、Zangyd\Util\ZangydHttp\Http\Psr7\UploadedFile，其中键值会作为html编码，文件则是上传）
     * @var mixed
     */
    public $content;
    ...
    {
        $this->handler = ZangydHttp::getHandler();
        $this->retry = 0;
    ...
    public function send($url = null, $requestBody = null, $method = null, $contentType = null)
    {
        ...
        return ZangydHttp::send($request, $this->handler);
    }
    ...
    public function websocket($url = null)
    {
        ...
        return ZangydHttp::websocket($request, $this->handler);
    }
